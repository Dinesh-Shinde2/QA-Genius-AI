...
export const useAppStore = create<AppState>((set, get) => ({
 ...
 unreadNotificationCount: 0,
  ...
  fetchSprints: async (projectId) => {
    ...
    if (!token) return;
    try {
      const res = await axios.get(`${API_BASE_URL}/api/sprints/${projectId}`, {
  ...
  createSprint: async (sprintData) => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/api/sprints/`, sprintData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      await get().fetchSprints(sprintData.project_id);
      return true;
  ...
  fetchReleases: async (projectId) => {
    ...
    if (!token) return;
    try {
      const res = await axios.get(`${API_BASE_URL}/api/releases/${projectId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      set({ releases: res.data });
    } catch (err) { console.error(err); }
  },
  ...
  createRelease: async (releaseData) => {
    ...
    if (!token) return false;
    try {
      await axios.post(`${API_BASE_URL}/api/releases/`, releaseData, {
        headers: { Authorization: `Bearer ${token}` }
      });
      await get().fetchReleases(releaseData.project_id);
      return true;
    } catch (err) { return false; }
  },
