...
export default function Dashboard() {
 ...
 const statCards = [
  { name: 'Total Projects', value: projects.length, icon: FolderRoot, color: 'text-[#2F81F7]' },
  { name: 'Open Bugs', value: bugs.filter(b => b.status === 'OPEN').length, icon: FileSpreadsheet, color: 'text-orange-400' },
  ...
  { name: 'Test Execution', value: '82%', icon: TrendingUp, color: 'text-emerald-500' },
 ];
   ...
   <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 md:grid-cols-5 gap-4">
     {statCards.map((card) => {
      ...
      return (
       <div key={card.name} className="bg-white border border-slate-200 shadow-sm p-5 rounded-2xl flex items-center justify-between border border-slate-200 hover:border-[#2F81F7]/20 hover:shadow-lg hover:shadow-blue-500/5 group hover:-translate-y-0.5 transition-all duration-300">
        ...
        <div className="p-2 rounded-lg bg-slate-50 border border-slate-200 group-hover:border-slate-200 transition duration-300">
    ...
    {activeProject ? (
     <div className="grid grid-cols-1 lg:grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      
      {/* Module Coverage Graph */}
      <div className="lg:col-span-2 bg-white border border-slate-200 shadow-sm p-5 rounded-xl flex flex-col gap-4">
       <div>
        <h3 className="text-sm font-semibold text-slate-900">Test Execution Summary</h3>
        <p className="text-[11px] text-slate-500">Pass / Fail / Blocked metrics across modules.</p>
       </div>
      ...
      <div className="bg-white border border-slate-200 shadow-sm p-5 rounded-xl flex flex-col gap-4">
       <div>
        <h3 className="text-sm font-semibold text-slate-900">Bug Severity Summary</h3>
        <p className="text-[11px] text-slate-500">Proportions of suggested failure vectors.</p>
       </div>
