    ...
    <div className="flex h-screen overflow-hidden bg-[#08060f]">
      ...
      <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Requirement Coverage Matrix</h1>
        ...
        {activeProject && total > 0 && (
          <div className="glass-panel p-5 rounded-xl flex items-center justify-between">
            <div className="flex flex-col gap-1">
              <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Overall Coverage Index</span>
            ...
            {/* Visual Bar progress */}
            <div className="w-64 h-3 bg-slate-900 rounded-full overflow-hidden border border-slate-800">
              <div 
                ...
                {(['ALL', 'COVERED', 'MISSING'] as const).map((status) => (
                  <button
                    ...
                    className={`px-2.5 py-1 rounded border transition ${
                      statusFilter === status
                        ? 'bg-violet-950/40 border-violet-500 text-violet-300 font-semibold'
                        : 'bg-transparent border-slate-850 text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    {status}
                  </button>
