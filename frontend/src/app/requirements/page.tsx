...
export default function RequirementsPage() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    setStatusMsg('Running AI QA Package Generation (Business Rules, Test Cases & Bugs)...');
    ...
    <div className="flex h-screen overflow-hidden bg-[#08060f]">
      ...
      <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div>
            <h1 className="text-2xl font-bold text-white">QA Package Generator</h1>
        ...
        {activeProject ? (
          <div className="grid grid-cols-1 xl:grid-cols-5 gap-6 items-start">
            ...
            <div className="xl:col-span-2 flex flex-col gap-5">
              <form onSubmit={handleGenerate} className="glass-panel p-6 rounded-xl flex flex-col gap-4">
                <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
                  <Settings2 className="w-5 h-5 text-violet-400" />
                  <h3 className="font-semibold text-white text-sm">Generator Configuration</h3>
                </div>
                ...
                {/* Upload Field */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-300">System Spec File / Screenshot</label>
                  <div className="border border-dashed border-slate-700 hover:border-violet-500/50 rounded-lg p-4 flex flex-col items-center justify-center gap-2 cursor-pointer transition bg-slate-900/10">
                    <input 
                      type="file" 
                      id="prd-upload"
                ...
                {/* Pasted text */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-slate-300">User Story Description</label>
                  <textarea 
                    value={pastedText}
                ...
                <div className="flex flex-col gap-2.5">
                  <label className="text-xs font-semibold text-slate-300">Smart Test Case Filters</label>
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-lg text-sm font-semibold bg-gradient-to-r from-violet-600 to-cyan-600 hover:from-violet-500 hover:to-cyan-500 text-white shadow-lg shadow-violet-500/20 transition duration-200 disabled:opacity-50 mt-2 flex items-center justify-center gap-2"
                >
            ...
            <div className="xl:col-span-3 flex flex-col gap-4 min-h-[500px]">
              ...
              {isUploading && (
                <div className="flex-1 glass-panel rounded-xl flex flex-col items-center justify-center p-8 text-center gap-4">
                  <div className="w-12 h-12 rounded-full border-4 border-slate-800 border-t-violet-500 animate-spin" />
                  <div className="flex flex-col gap-1">
                    <h3 className="text-sm font-bold text-white">Processing Document</h3>
                    <p className="text-xs text-slate-400 font-mono animate-pulse">{statusMsg}</p>
                  </div>
                </div>
              )}
              ...
              {!isUploading && !packageResult && (
                <div className="flex-1 glass-panel rounded-xl flex flex-col items-center justify-center p-8 text-center gap-4">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-white">No QA Package Loaded</h3>
                ...
                <div className="flex-1 flex flex-col gap-4 animate-in fade-in duration-200">
                  ...
                  <div className="flex border-b border-slate-800 gap-2">
                    <button
                      ...
                      className={`px-4 py-2 text-xs font-semibold border-b-2 transition ${
                        activeTab === 'analysis' ? 'border-violet-500 text-violet-400' : 'border-transparent text-slate-400 hover:text-slate-200'
                      }`}
                    >
                      1. Requirement Specs
                    </button>
                    <button
                      ...
                      className={`px-4 py-2 text-xs font-semibold border-b-2 transition ${
                        activeTab === 'testcases' ? 'border-violet-500 text-violet-400' : 'border-transparent text-slate-400 hover:text-slate-200'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-4 py-2 text-xs font-semibold border-b-2 transition ${
                        activeTab === 'bugs' ? 'border-violet-500 text-violet-400' : 'border-transparent text-slate-400 hover:text-slate-200'
                      }`}
                    >
                  ...
                  {activeTab === 'analysis' && (
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="glass-panel p-5 rounded-xl flex flex-col gap-2">
                        <h4 className="text-xs font-bold text-cyan-400 uppercase tracking-wide">Extracted Summary</h4>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="glass-panel p-4 rounded-xl flex flex-col gap-2">
                          <h4 className="text-xs font-bold text-violet-400 uppercase tracking-wide">Business Rules</h4>
                        ...
                        <div className="glass-panel p-4 rounded-xl flex flex-col gap-2">
                          ...
                          <ul className="list-decimal list-inside text-xs text-slate-300 flex flex-col gap-1.5">
                        ...
                        <table className="w-full text-left border-collapse">
                          ...
                          <tbody className="divide-y divide-slate-800 text-xs text-slate-300">
                            {packageResult.testcases.map((tc: any) => {
                              ...
                              return (
                                <tr key={tc.id} className="hover:bg-slate-900/10">
                                  <td className="p-3 font-mono font-bold text-violet-400">{tc.custom_id}</td>
                                  <td className="p-3">
                                    <div className="font-semibold text-white">{tc.feature}</div>
                                    <div className="text-slate-400 mt-0.5">{tc.scenario}</div>
                                  </td>
                                  <td className="p-3"><span className="px-2 py-0.5 rounded bg-slate-800 text-[10px] text-slate-400">{tc.case_type}</span></td>
                  ...
                  {activeTab === 'bugs' && packageResult.bugs.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 flex-1">
                      ...
                      <div className="flex flex-col gap-1.5 border-r border-slate-800 pr-4">
                          ...
                          <button
                            ...
                            className={`w-full text-left p-2.5 rounded-lg border text-xs transition duration-150 ${
                              activeBugIndex === idx
                                ...
                                : 'bg-transparent border-slate-800 text-slate-400 hover:bg-slate-900/20'
                            }`}
                          >
                            <div className="font-mono font-bold text-[10px] text-rose-400">{bug.custom_id}</div>
                            <div className="font-medium truncate mt-0.5 text-slate-200">{bug.title}</div>
                          </button>
                        ))}
                      </div>
                      ...
                      <div className="md:col-span-3 flex flex-col gap-4">
                        ...
                        {/* Format Switcher */}
                        <div className="flex justify-between items-center bg-slate-900/30 p-2 rounded-lg border border-slate-800">
                        ...
                        {bugFormat === 'enterprise' && (
                          <div className="glass-panel p-5 rounded-xl flex flex-col gap-4 text-xs text-slate-300">
                            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                              <h3 className="font-bold text-white text-sm">{packageResult.bugs[activeBugIndex].title}</h3>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                ...
                                <p className="mt-1 whitespace-pre-line bg-slate-950/20 p-2.5 rounded border border-slate-800 font-mono text-[11px]">
                              ...
                              <div>
                                ...
                                <p className="mt-1 bg-slate-950/20 p-2.5 rounded border border-slate-800">
                            ...
                            <div className="grid grid-cols-2 gap-4 border-t border-slate-800 pt-3">
                              <div>
                                ...
                                <p className="mt-1 text-slate-300">{packageResult.bugs[activeBugIndex].impact_analysis}</p>
                              </div>
                              <div>
                                <span className="text-[10px] text-slate-500 uppercase font-mono">Suggested Fix (Dev)</span>
                                <p className="mt-1 text-cyan-300 font-mono text-[11px]">{packageResult.bugs[activeBugIndex].root_cause_suggestion}</p>
                              </div>
                            </div>
                          </div>
                        )}
                        ...
                        {bugFormat === 'jira' && (
                          <div className="glass-panel p-4 rounded-xl flex flex-col gap-3">
                            <div className="flex justify-between items-center">
                              <span className="text-xs text-slate-400">Raw JIRA markdown:</span>
                              <button
                                onClick={() => triggerCopy(packageResult.bugs[activeBugIndex].jira_format?.markdown || '')}
                                className="px-2 py-1 rounded bg-slate-800 text-[10px] text-slate-300 flex items-center gap-1.5 hover:bg-slate-700"
                              >
                                {copied ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                                {copied ? 'Copied' : 'Copy'}
                              </button>
                            </div>
                            <pre className="p-3 bg-slate-950/50 rounded-lg text-[11px] font-mono text-slate-300 overflow-x-auto whitespace-pre-wrap max-h-80 border border-slate-800">
                        ...
                        {bugFormat === 'developer' && (
                          <div className="glass-panel p-4 rounded-xl flex flex-col gap-3">
                            <div className="flex justify-between items-center">
                              <span className="text-xs text-slate-400">Developer JSON metadata:</span>
                              <button
                            ...
                            <pre className="p-3 bg-slate-950/50 rounded-lg text-[11px] font-mono text-cyan-400 overflow-x-auto border border-slate-800">
          ...
          <div className="flex-1 glass-panel rounded-2xl flex flex-col items-center justify-center p-12 text-center gap-4 border border-dashed border-slate-800">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">No Project Loaded</h2>
