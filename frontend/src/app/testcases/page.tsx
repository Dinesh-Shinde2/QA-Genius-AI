...
export default function TestCasesPage() {
 ...
 const getCaseTypeBadgeClass = (caseType: string) => {
  switch (caseType?.toLowerCase()) {
   ...
   case 'boundary': return 'bg-slate-500/10 text-slate-500 border border-slate-500/20';
   case 'edge case': return 'bg-slate-500/10 text-slate-500 border border-slate-500/20';
   default: return 'bg-slate-500/10 text-slate-500 border border-slate-500/20';
  }
 };
  ...
  <div className="flex min-min-h-screen bg-slate-50">
   ...
   <div className="flex-1 overflow-y-auto p-8 flex flex-col gap-6">
    ...
    <div className="flex items-center justify-between border-b border-slate-200 pb-4">
     <div>
      <h1 className="text-2xl font-bold text-slate-900">Manual Test Cases</h1>
     ...
     {activeProject && (
      <div className="flex gap-2">
       <button
        ...
        className="px-4 py-1.5 rounded-lg bg-[#2F81F7] hover:from-blue-500 hover:to-slate-500 text-slate-900 font-bold text-xs flex items-center gap-1.5 transition duration-200 shrink-0"
       >
       ...
       <button
        ...
        className={`px-3 py-1.5 rounded-lg border text-xs font-bold flex items-center gap-1.5 transition duration-200 shrink-0 ${
         adoConfigured 
          ? 'bg-[#2F81F7]/10 border-[#2F81F7]/35 text-blue-300 hover:bg-[#2F81F7]/20' 
          : 'bg-slate-800 border-slate-700 text-slate-500 hover:bg-slate-750 hover:text-slate-800'
        }`}
       >
       ...
       <button
        ...
        className="px-3 py-1.5 rounded-lg bg-white border border-slate-200 shadow-sm text-xs text-slate-700 hover:text-slate-900 flex items-center gap-1.5 transition"
       >
       ...
       <button
        ...
        className="px-3 py-1.5 rounded-lg bg-white border border-slate-200 shadow-sm text-xs text-emerald-400 hover:text-emerald-300 flex items-center gap-1.5 border-emerald-950/20 transition"
       >
      ...
      <div className="flex gap-3 w-full md:w-auto justify-end text-xs">
       <div className="flex flex-col gap-1">
        ...
        <select
         ...
         className="glass-input px-2.5 py-1.5 bg-[#120e25] text-slate-900"
        >
        ...
        <tbody className="divide-y divide-slate-800 text-xs text-slate-700">
         {filteredCases.length > 0 ? (
          filteredCases.map((tc) => {
           ...
           return (
            <tr key={tc.id} className="hover:bg-white/15 transition-colors border-b border-slate-200/50">
             <td className="p-3 font-mono font-bold text-[#2F81F7]">{tc.custom_id}</td>
             <td className="p-3 font-medium text-slate-500">{tc.module}</td>
             <td className="p-3 font-semibold text-slate-800">{tc.feature}</td>
             <td className="p-3 max-w-xs truncate text-slate-700 font-medium">{tc.scenario}</td>
             <td className="p-3">
             ...
             <td className="p-3 text-right">
              <button
               ...
               className="p-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-500 hover:text-slate-700 transition"
              >
   ...
   {activeTC && (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
     <div className="bg-white border border-slate-200 shadow-sm w-full max-w-2xl p-6 rounded-xl flex flex-col gap-4 animate-in fade-in zoom-in duration-150 max-h-[90vh] overflow-y-auto">
      <div className="flex items-center justify-between border-b border-slate-200 pb-3">
       <div className="flex items-center gap-2">
        <span className="font-mono font-bold text-[#2F81F7] text-sm">{activeTC.custom_id}</span>
        <span className="text-[10px] px-2 py-0.5 rounded bg-slate-800 text-slate-500 uppercase font-semibold font-mono">
         ...
         <>
          {adoConfigured ? (
           <button
            ...
            className="px-2.5 py-1 rounded bg-[#2F81F7]/30 hover:bg-[#2F81F7]/80 border border-[#2F81F7]/30 text-blue-300 hover:text-slate-900 text-[10px] font-bold uppercase tracking-wider transition flex items-center gap-1 disabled:opacity-50"
           >
          ...
          <button
           ...
           className="px-2.5 py-1 rounded bg-slate-800 hover:bg-slate-700 border border-slate-200 text-[#2F81F7] text-[10px] font-bold uppercase transition"
          >
           Edit Fields
          </button>
         </>
      ...
      {editingTC ? (
       <form onSubmit={handleSaveEditTC} className="flex flex-col gap-4 text-xs text-slate-700">
        {editError && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         <div className="flex flex-col gap-1">
          <label className="text-[10px] uppercase font-mono tracking-wider text-slate-500">Module *</label>
          <input 
           type="text" 
           required
         ...
         <div className="flex flex-col gap-1">
          <label className="text-[10px] uppercase font-mono tracking-wider text-slate-500">Feature Scope *</label>
          <input 
           type="text" 
           required
        ...
        <div className="flex flex-col gap-1">
         <label className="text-[10px] uppercase font-mono tracking-wider text-slate-500">Scenario Summary *</label>
         <input 
          type="text" 
          required
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
         <div className="flex flex-col gap-1">
          <label className="text-[10px] uppercase font-mono tracking-wider text-slate-500">Priority</label>
          <select
           value={editingTC.priority}
           onChange={(e) => setEditingTC({ ...editingTC, priority: e.target.value })}
           className="w-full glass-input px-3 py-2 text-xs bg-[#120e25] text-slate-900"
          >
         ...
         <div className="flex flex-col gap-1">
          <label className="text-[10px] uppercase font-mono tracking-wider text-slate-500">Case Type</label>
          <select
        ...
        <div className="flex flex-col gap-1">
         <label className="text-[10px] uppercase font-mono tracking-wider text-slate-500">Preconditions</label>
         <textarea 
        ...
        <div className="flex flex-col gap-1">
         <label className="text-[10px] uppercase font-mono tracking-wider text-slate-500">Steps (One per line) *</label>
         <textarea 
          required
        ...
        <div className="flex flex-col gap-1">
         <label className="text-[10px] uppercase font-mono tracking-wider text-slate-500">Required Test Data</label>
         <input 
          type="text" 
          value={editingTC.test_data}
          onChange={(e) => setEditingTC({ ...editingTC, test_data: e.target.value })}
          className="w-full glass-input px-3 py-2 text-xs font-mono text-slate-500"
         />
        </div>
        ...
        <div className="flex flex-col gap-1.5 border-t border-slate-200/80 pt-4 mt-2">
         <label className="text-[10px] uppercase font-mono tracking-wider text-slate-700 font-bold">Select Publication State Preset</label>
        ...
        <div className="flex justify-end gap-3 mt-4 border-t border-slate-200 pt-4 font-semibold">
         <button 
          type="button"
         ...
         <button 
          type="submit"
          className="px-5 py-2 text-xs font-bold rounded-lg bg-emerald-600 hover:from-emerald-500 hover:to-teal-500 text-slate-900 flex items-center gap-1.5"
          disabled={saving}
         >
       ...
       <div className="flex flex-col gap-4 overflow-y-auto max-h-[450px] pr-2 text-xs text-slate-700">
        <div>
         <span className="text-[10px] text-slate-500 uppercase font-mono">Feature Scope</span>
         <p className="font-semibold text-slate-900 mt-0.5">{activeTC.feature}</p>
        </div>
        ...
        {activeTC.preconditions && (
         <div>
          ...
          <p className="bg-slate-50/20 p-2 rounded border border-slate-200/60 mt-0.5">{activeTC.preconditions}</p>
         </div>
        )}

        <div>
         <span className="text-[10px] text-slate-500 uppercase font-mono">Reproduction Verification Steps</span>
         <ol className="list-decimal list-inside flex flex-col gap-1.5 mt-1 bg-slate-50/30 p-3 rounded-lg border border-slate-200/80 font-mono text-[11px] text-slate-800">
         ...
         <div>
          ...
          {activeTC.test_data.startsWith('Synced to ADO') ? (
           <div className="bg-blue-950/10 border border-blue-900/30 p-2.5 rounded-lg text-blue-300 font-mono text-[10px] flex items-center justify-between mt-0.5 animate-pulse">
            ...
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-[#2F81F7]/10 text-[#2F81F7] uppercase font-bold border border-[#2F81F7]/20">
             Linked Board Work Item
            </span>
           </div>
          ) : (
           <p className="font-mono bg-slate-50/20 p-2 rounded border border-slate-200/60 mt-0.5 text-slate-500">{activeTC.test_data}</p>
          )}
         </div>
        )}
   ...
   {showGenModal && (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
     {previewCases.length === 0 ? (
      ...
      <div className="bg-white border border-slate-200 shadow-sm w-full max-w-lg p-6 rounded-xl flex flex-col gap-4 animate-in fade-in zoom-in duration-200">
       <div className="flex justify-between items-start border-b border-slate-200 pb-3">
        <div>
         <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
          <Plus className="w-5 h-5 text-[#2F81F7] animate-pulse" /> Agile AI Test Case Builder
         </h2>
       ...
       <form onSubmit={handleGenerateManualTestCases} className="flex flex-col gap-4 mt-1">
        ...
        <div className="flex flex-col gap-1.5">
         <label className="text-xs font-semibold text-slate-700">Module / Epic Scope</label>
         <input 
          type="text" 
        ...
        <div className="flex flex-col gap-1.5">
         <label className="text-xs font-semibold text-slate-700">User Story Description / Acceptance Criteria</label>
         <textarea 
        ...
        <div className="flex flex-col gap-1.5">
         <label className="text-xs font-semibold text-slate-700">Case Types to Generate</label>
          ...
          {['Positive', 'Negative', 'Boundary', 'Edge Case'].map(type => (
           <label key={type} className="flex items-center gap-2 text-xs text-slate-700 cursor-pointer">
            <input 
             ...
             className="rounded border-slate-200 bg-slate-50/40 text-[#2F81F7]"
            />
            {type}
           </label>
          ))}
        ...
        <div className="flex justify-end gap-2 mt-3 pt-3 border-t border-slate-200">
         ...
         <button 
          type="submit"
          className="px-4 py-2 text-xs font-bold rounded-lg bg-[#2F81F7] hover:from-blue-500 hover:to-slate-500 text-slate-900 flex items-center gap-1.5 disabled:opacity-50"
          disabled={generating}
         >
          {generating ? (
           <>
      ...
      <div className="bg-white border border-slate-200 shadow-sm w-full max-w-5xl p-6 rounded-xl flex flex-col gap-4 animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
       <div className="flex justify-between items-start border-b border-slate-200 pb-3">
        <div>
         <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
       ...
       <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mt-1 items-start min-h-[400px]">
        ...
        <div className="md:col-span-2 flex flex-col gap-3 max-h-[450px] overflow-y-auto pr-2 border-r border-slate-200/80">
         <div className="flex justify-between items-center bg-slate-50/20 p-2 rounded border border-slate-200">
          ...
          <div className="flex gap-2">
           <button
            ...
            className="text-[9px] text-[#2F81F7] font-bold hover:text-blue-300"
           >
            All
           </button>
           ...
           <button
            ...
            className="text-[9px] text-slate-500 font-bold hover:text-slate-700"
           >
            None
           </button>
          </div>
           ...
           <div 
            ...
            className={`flex items-start gap-2.5 p-3.5 rounded-lg border text-left cursor-pointer transition-all duration-200 ${
             triageActiveIndex === idx
              ? 'bg-gradient-to-br from-blue-950/30 to-slate-950/20 border-[#2F81F7]/40 text-blue-300 translate-x-1'
              : 'bg-transparent border-slate-200 hover:bg-white/10 hover:border-slate-200 text-slate-500 hover:text-slate-800'
            }`}
           >
            <input
             type="checkbox"
             ...
             className="mt-0.5 rounded border-slate-200 bg-slate-50/40 text-[#2F81F7]"
            />
            <div className="flex-grow min-w-0">
             <div className="flex justify-between items-center text-[9px]">
              <span className="font-mono text-[#2F81F7] uppercase font-bold">{tc.case_type}</span>
              <span className="text-slate-500">{tc.priority}</span>
             </div>
             <div className="text-[11px] font-semibold text-slate-800 mt-1 truncate">{tc.scenario}</div>
            </div>
           </div>
          ))}
         </div>
        ...
        <div className="md:col-span-3 flex flex-col gap-4 max-h-[450px] overflow-y-auto pr-2 bg-slate-50/15 p-4 rounded-xl border border-slate-200">
         {previewCases[triageActiveIndex] ? (
          <div className="flex flex-col gap-4 text-xs text-slate-700 animate-in fade-in duration-200">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
             ...
             <p className="font-semibold text-slate-900 mt-0.5">{previewCases[triageActiveIndex].feature}</p>
            </div>
            <div>
             <span className="text-[10px] text-slate-500 uppercase font-mono">Validation Type</span>
             <p className="font-semibold text-[#2F81F7] mt-0.5 font-mono">{previewCases[triageActiveIndex].case_type}</p>
            </div>
           </div>
           ...
           {previewCases[triageActiveIndex].preconditions && (
            <div>
             <span className="text-[10px] text-slate-500 uppercase font-mono">Preconditions</span>
             <p className="bg-slate-50/20 p-2 rounded border border-slate-200/60 mt-0.5">
              {previewCases[triageActiveIndex].preconditions}
             </p>
            </div>
           )}
           ...
           <div>
            ...
            <pre className="mt-1 whitespace-pre-line bg-slate-50/20 p-3 rounded border border-slate-200/80 font-mono text-[10px] leading-relaxed text-slate-700">
       ...
       <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-t border-slate-200 pt-4 mt-2">
        <div className="flex items-center gap-3">
         <span className="text-xs text-slate-500 font-semibold font-mono uppercase">Publication Preset:</span>
         <div className="flex gap-2">
          <button
           type="button"
           ...
           className={`px-3 py-1 rounded text-[10px] font-bold transition ${
            bulkSaveStatus === 'DRAFT'
             ...
             : 'bg-slate-800 text-slate-500 hover:text-slate-800'
           }`}
          >
           DRAFT
          </button>
        ...
        <div className="flex gap-3">
         ...
         <button 
          ...
          className="px-5 py-2 text-xs font-bold rounded-lg bg-emerald-600 hover:from-emerald-500 hover:to-teal-500 text-slate-900 flex items-center gap-1.5 disabled:opacity-50"
     ...
     <div className="bg-white border border-slate-200 shadow-sm w-full max-w-md p-6 rounded-xl flex flex-col gap-4 animate-in fade-in zoom-in duration-200">
      <div className="flex justify-between items-start border-b border-slate-200 pb-3">
       <div>
        <h2 className="text-lg font-bold text-slate-900 flex items-center gap-1.5">
         <Settings2 className="w-5 h-5 text-[#2F81F7]" /> Azure DevOps Connect
        </h2>
        <p className="text-xs text-slate-500 mt-1">Configure Azure DevOps board mappings for this project.</p>
       </div>
       <button 
      ...
      <form onSubmit={handleSaveADOSettings} className="flex flex-col gap-4">
       ...
       <div className="flex justify-end gap-2 mt-2 pt-2 border-t border-slate-200">
        ...
        <button 
         type="submit"
         className="px-4 py-2 text-xs font-bold rounded-lg bg-[#2F81F7] hover:from-blue-500 hover:to-slate-500 text-slate-900 "
        >
         Save Connection
        </button>
       </div>
