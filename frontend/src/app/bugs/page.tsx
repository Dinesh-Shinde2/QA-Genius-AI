...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; border: string; icon: any; dot: string }> = {
 NEW:        { label: 'New',       color: 'text-slate-700', bg: 'bg-slate-800/60',   border: 'border-slate-600/40', icon: CircleDot,   dot: 'bg-slate-400' },
 OPEN:       { label: 'Open',       color: 'text-blue-300',  bg: 'bg-blue-950/40',    border: 'border-blue-700/40',  icon: Bug,      dot: 'bg-blue-400' },
 ASSIGNED:     { label: 'Assigned',     color: 'text-slate-700', bg: 'bg-slate-50/40',   border: 'border-slate-700/40', icon: User,     dot: 'bg-slate-400' },
 ...
 READY_FOR_RETEST: { label: 'Ready for Retest', color: 'text-slate-700',  bg: 'bg-slate-50/30',    border: 'border-slate-700/40',  icon: ClipboardCheck,dot: 'bg-slate-400' },
 RETESTING:     { label: 'Retesting',    color: 'text-blue-300', bg: 'bg-blue-950/30',   border: 'border-blue-700/40', icon: RotateCcw,   dot: 'bg-blue-400' },
...
function BugCard({ bug, onClick }: { bug: any; onClick: () => void }) {
 return (
  <div
   onClick={onClick}
   className="bg-white/90 border border-slate-200/60 rounded-xl p-3.5 cursor-pointer hover:border-blue-700/40 transition-all duration-150 group active:scale-98 shadow-sm"
  >
   <div className="flex items-start justify-between gap-2 mb-2">
    <span className="text-[10px] font-mono text-slate-500 font-bold">{bug.custom_id}</span>
    <SeverityBadge severity={bug.severity} />
   </div>
   <p className="text-xs font-semibold text-slate-800 line-clamp-2 leading-relaxed mb-2 group-hover:text-slate-900">{bug.title}</p>
...
function BugRow({ bug, onClick }: { bug: any; onClick: () => void }) {
 return (
  <tr onClick={onClick} className="border-b border-slate-200/60 hover:bg-white/20 cursor-pointer transition group">
   <td className="px-4 py-3 text-[10px] font-mono text-slate-500 font-bold whitespace-nowrap">{bug.custom_id}</td>
   <td className="px-4 py-3 max-w-xs">
    <p className="text-xs font-semibold text-slate-800 truncate group-hover:text-slate-900">{bug.title}</p>
   ...
   <td className="px-4 py-3">
    {bug.assigned_to_name ? (
     <div className="flex items-center gap-1.5">
      ...
      <span className="text-[10px] text-slate-500 truncate max-w-[80px]">{bug.assigned_to_name}</span>
     </div>
...
function BugsPageContent() {
 ...
 const inputCls = 'w-full bg-white/60 border border-slate-700/60 rounded-lg px-3 py-2 text-sm text-slate-900 placeholder-slate-500 focus:outline-none focus:border-[#2F81F7]/60 transition';
    ...
    <div className="col-span-2">
     ...
     {form.tags.length > 0 && (
      <div className="flex flex-wrap gap-1.5 mt-2">
       {form.tags.map((tag, i) => (
        <span key={i} className="flex items-center gap-1 px-2 py-0.5 bg-blue-950/40 border border-blue-700/40 text-blue-300 text-[10px] rounded-full font-semibold">
 ...
 const TimelineItem = ({ item }: { item: any }) => {
  const actionColors: Record<string, string> = {
   CREATED: 'bg-[#2F81F7]', ASSIGNED: 'bg-slate-500', STATUS_CHANGED: 'bg-amber-500',
   UPDATED: 'bg-slate-500', COMMENT_ADDED: 'bg-[#2F81F7]', REOPENED: 'bg-rose-500', CLOSED: 'bg-green-500',
  };
  return (
   <div className="flex gap-3 relative">
    ...
    <div className="pb-4 flex-1 min-w-0">
     <p className="text-xs text-slate-700 leading-relaxed">{item.description || `${item.action?.replace(/_/g, ' ')}`}</p>
     {item.old_value && item.new_value && (
      <div className="flex items-center gap-2 mt-1">
       <span className="text-[10px] text-slate-600 line-through">{item.old_value}</span>
       <ChevronRight className="w-3 h-3 text-slate-600" />
       <span className="text-[10px] text-slate-500 font-semibold">{item.new_value}</span>
      </div>
     )}
     <div className="flex items-center gap-2 mt-1">
 ...
 const renderDashboard = () => {
  ...
  if (!d) return <div className="flex items-center justify-center h-64"><Loader2 className="w-6 h-6 text-[#2F81F7] animate-spin" /></div>;
  const statsCards = [
   { label: 'Total Bugs', value: d.total, icon: Bug, color: 'text-[#2F81F7]', bg: 'from-blue-950/40 to-slate-950/30' },
   { label: 'Open', value: d.open, icon: CircleDot, color: 'text-[#2F81F7]', bg: 'from-blue-950/40 to-blue-950/20' },
   { label: 'In Progress', value: d.in_progress, icon: Loader2, color: 'text-amber-400', bg: 'from-amber-950/40 to-amber-950/20' },
   { label: 'Ready for Retest', value: d.ready_for_retest, icon: ClipboardCheck, color: 'text-slate-500', bg: 'from-slate-950/40 to-slate-950/20' },
   ...
   { label: 'Avg Resolution', value: `${d.avg_resolution_hours}h`, icon: Clock3, color: 'text-[#2F81F7]', bg: 'from-blue-950/40 to-blue-950/20' },
  ];
  return (
   <div className="p-6 space-y-6">
    {/* Stats Grid */}
    <div className="grid grid-cols-2 md:grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
     {statsCards.map((s) => {
      const Icon = s.icon;
      return (
       <div key={s.label} className={`bg-gradient-to-br ${s.bg} border border-slate-200/60 rounded-2xl p-4`}>
    ...
    <div className="grid grid-cols-1 md:grid-cols-1 md:grid-cols-2 gap-6">
     ...
     <div className="bg-white/80 border border-slate-200/60 rounded-2xl p-5">
      <h3 className="text-xs font-bold text-slate-700 mb-4 uppercase tracking-widest">Severity Distribution</h3>
     ...
     <div className="bg-white/80 border border-slate-200/60 rounded-2xl p-5">
      <h3 className="text-xs font-bold text-slate-700 mb-4 uppercase tracking-widest">Module-wise Defects</h3>
      <ResponsiveContainer width="100%" height={180}>
    ...
    {d.developer_performance?.length > 0 && (
     <div className="bg-white/80 border border-slate-200/60 rounded-2xl p-5">
      <h3 className="text-xs font-bold text-slate-700 mb-4 uppercase tracking-widest">Developer Performance</h3>
      <div className="space-y-3">
       {d.developer_performance.map((dev: any) => (
        <div key={dev.name} className="flex items-center gap-4">
         <Avatar name={dev.name} size="md" />
         <span className="text-xs text-slate-700 w-24 truncate">{dev.name}</span>
         <div className="flex-1 bg-white/60 rounded-full h-1.5">
          <div className="bg-[#2F81F7] h-1.5 rounded-full transition-all" style={{ width: `${dev.total_assigned ? (dev.resolved / dev.total_assigned) * 100 : 0}%` }} />
         </div>
         <div className="flex gap-3 text-[10px] shrink-0">
          <span className="text-slate-500">{dev.total_assigned} assigned</span>
 ...
 return (
  <div className="flex min-min-h-screen bg-background text-foreground transition-colors duration-300">
   <Sidebar />

   <main className="flex-1 flex flex-col pt-14 md:pt-0 overflow-x-hidden overflow-hidden">
    {/* ── Top Bar ─────────────────────────────────────────────────────── */}
    <div className="shrink-0 border-b border-slate-200 bg-slate-50/50 px-5 py-3 flex items-center gap-3">
     {/* View Toggle */}
     <div className="flex bg-white/60 border border-slate-200/60 rounded-xl p-0.5 gap-0.5">
      {[['kanban', LayoutGrid], ['list', List], ['dashboard', BarChart2]].map(([v, Icon]) => (
       <button key={v as string} onClick={() => setView(v as ViewMode)}
        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition ${view === v ? 'bg-[#2F81F7] text-slate-900 ' : 'text-slate-500 hover:text-slate-700'}`}>
     ...
     <div className="relative flex-1 max-w-xs">
      ...
      <input value={filterSearch} onChange={e => setFilterSearch(e.target.value)} placeholder="Search bugs..."
       className="w-full bg-white/60 border border-slate-200/60 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-900 placeholder-slate-500 focus:outline-none focus:border-[#2F81F7]/60 transition" />
     </div>
     ...
     <div className="ml-auto flex items-center gap-2">
      ...
      <button onClick={() => setShowAIModal(true)}
       className="flex items-center gap-1.5 px-3 py-2 bg-[#2F81F7] hover:from-blue-500 hover:to-slate-500 text-slate-900 rounded-xl text-xs font-bold transition active:scale-95">
       <Bot className="w-3.5 h-3.5" />
       AI Generate
      </button>
      <button onClick={() => { setForm({ ...EMPTY_FORM }); setShowCreateModal(true); }}
       className="flex items-center gap-1.5 px-3 py-2 bg-[#2F81F7] hover:bg-[#2F81F7] text-slate-900 rounded-xl text-xs font-bold transition active:scale-95">
         ...
         return (
          <div key={ci} className="flex-1 min-w-[260px] max-w-[300px] flex flex-col">
           <div className="flex items-center justify-between mb-3 px-1">
            <div>
             <h3 className="text-xs font-black text-slate-700">{KANBAN_COL_LABELS[ci]}</h3>
   ...
   {drawerOpen && activeBug && (
    <div className="fixed inset-0 z-40 flex">
     ...
     {/* Drawer Panel */}
     <div className="w-[840px] max-w-[90vw] h-full bg-slate-50 border-l border-slate-200 flex flex-col shadow-2xl animate-in slide-in-from-right duration-250">

      {/* Drawer Header */}
      <div className="shrink-0 border-b border-slate-200 px-5 py-4 flex items-start justify-between gap-4">
       <div className="flex-1 min-w-0">
        ...
        <h2 className="text-sm font-black text-slate-900 line-clamp-1">{activeBug.title}</h2>
       </div>
       ...
       <div className="flex items-center gap-2 shrink-0">
        ...
        <div className="relative">
         <button onClick={() => setShowStatusMenu(s => !s)}
          className="flex items-center gap-1.5 px-3 py-2 bg-slate-800/60 border border-slate-700/60 hover:border-slate-600 text-slate-700 rounded-lg text-xs font-semibold transition">
          <ArrowRightLeft className="w-3.5 h-3.5" />
          Status
          <ChevronDown className="w-3 h-3" />
         </button>
         {showStatusMenu && (
          <div className="absolute right-0 top-full mt-1.5 bg-white border border-slate-200 rounded-xl z-50 shadow-2xl py-1 w-52 animate-in fade-in slide-in-from-top-1 duration-100">
           <div className="px-3 pb-2 pt-1">
            <input placeholder="Optional comment..." value={statusComment} onChange={e => setStatusComment(e.target.value)}
             className="w-full bg-white/60 border border-slate-700/60 rounded-lg px-2 py-1.5 text-[10px] text-slate-900 placeholder-slate-600 focus:outline-none" />
           </div>
           {ALL_STATUSES.map(s => (
        ...
        <div className="relative">
         ...
         {showAssignPanel && (
          <div className="absolute right-0 top-full mt-1.5 bg-white border border-slate-200 rounded-xl z-50 shadow-2xl w-64 animate-in fade-in slide-in-from-top-1 duration-100">
           <div className="p-3 border-b border-slate-200">
            <input value={assignQuery} onChange={e => handleAssignSearch(e.target.value)} placeholder="Search user..."
             className="w-full bg-white/60 border border-slate-700/60 rounded-lg px-3 py-1.5 text-xs text-slate-900 placeholder-slate-500 focus:outline-none" />
           </div>
           <div className="max-h-48 overflow-y-auto py-1">
            {assignResults.map(u => (
             <button key={u.id} onClick={() => handleAssign(u)} className="w-full px-3 py-2 flex items-center gap-2 hover:bg-slate-800/50 transition text-left">
              <Avatar name={u.name} />
              <div>
               <p className="text-xs text-slate-700 font-semibold">{u.name}</p>
        ...
        <button onClick={() => { setEditMode(e => !e); }} className={`flex items-center gap-1.5 px-3 py-2 border rounded-lg text-xs font-semibold transition ${editMode ? 'bg-[#2F81F7] border-[#2F81F7] text-slate-900' : 'bg-slate-800/60 border-slate-700/60 text-slate-700 hover:border-slate-600'}`}>
        ...
        <button onClick={closeDrawer} className="p-2 rounded-lg hover:bg-slate-800/60 text-slate-500 transition">
         <X className="w-4 h-4" />
        </button>
       </div>
      </div>
      ...
      {/* Drawer Tabs */}
      <div className="shrink-0 border-b border-slate-200 flex gap-0 px-5">
       {[
        ...
        <button key={tab.id} onClick={() => setActiveTab(tab.id as any)}
         className={`flex items-center gap-1.5 px-4 py-3 text-xs font-bold border-b-2 transition ${activeTab === tab.id ? 'border-[#2F81F7] text-blue-300' : 'border-transparent text-slate-500 hover:text-slate-700'}`}>
      ...
      <div className="flex-1 overflow-y-auto">
       ...
       {activeTab === 'details' && (
        <div className="p-5">
         {/* Meta info row */}
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5 p-4 bg-white/30 rounded-xl border border-slate-200/50">
          <div>
           <p className={labelCls}>Created by</p>
           <div className="flex items-center gap-1.5">
            <Avatar name={activeBug.created_by_name} size="md" />
            <span className="text-xs text-slate-700">{activeBug.created_by_name || '—'}</span>
           </div>
          </div>
          <div>
           ...
           {activeBug.assigned_to_name ? (
            <div className="flex items-center gap-1.5">
             ...
             <div>
              <span className="text-xs text-slate-700">{activeBug.assigned_to_name}</span>
          ...
          <div>
           ...
           <span className="text-xs text-slate-500">{new Date(activeBug.created_at).toLocaleString()}</span>
          </div>
          <div>
           <p className={labelCls}>Updated</p>
           <span className="text-xs text-slate-500">{new Date(activeBug.updated_at).toLocaleString()}</span>
          </div>
         ...
         {editMode ? (
          <div>
           ...
           <div className="flex justify-end gap-2 mt-4">
            <button onClick={() => setEditMode(false)} className="px-4 py-2 text-xs text-slate-500 hover:text-slate-900">Cancel</button>
          ...
          <div className="space-y-5">
           {[
            ...
            <div key={f.label}>
             ...
             <div className="bg-white/40 rounded-lg px-3 py-2.5 border border-slate-200/50">
              <p className="text-xs text-slate-700 whitespace-pre-wrap leading-relaxed">{f.val}</p>
             </div>
            </div>
           ))}
          </div>
       ...
       {activeTab === 'comments' && (
        <div className="flex flex-col h-full">
          ...
          {(activeBug.comments || []).length === 0 ? (
           ...
           activeBug.comments!.map((c: any) => {
            ...
            const roleColors: Record<string, string> = {
             DEVELOPER: 'text-emerald-400', QA_ENGINEER: 'text-[#2F81F7]',
             QA_LEAD: 'text-slate-500', ADMIN: 'text-rose-400',
            };
            return (
             <div key={c.id} className={`flex gap-3 ${isOwn ? 'flex-row-reverse' : ''}`}>
              ...
              <div className={`max-w-[80%] ${isOwn ? 'items-end' : 'items-start'} flex flex-col gap-0.5`}>
               <div className="flex items-center gap-2">
                <span className={`text-[10px] font-bold ${roleColors[c.author_role] || 'text-slate-500'}`}>{c.author_name}</span>
                <span className="text-[10px] text-slate-600">{new Date(c.created_at).toLocaleTimeString()}</span>
               </div>
               <div className={`px-3.5 py-2.5 rounded-2xl text-xs text-slate-800 leading-relaxed ${isOwn ? 'bg-blue-950/50 border border-blue-700/30 rounded-tr-sm' : 'bg-white/60 border border-slate-200/60 rounded-tl-sm'}`}>
                {c.content}
               </div>
              </div>
             </div>
         ...
         {/* Comment Input */}
         <div className="shrink-0 border-t border-slate-200 p-4 flex gap-3 items-end">
          <Avatar name={user?.name} size="md" />
          <div className="flex-1 flex gap-2">
           <textarea value={commentText} onChange={e => setCommentText(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSendComment(); } }}
            placeholder="Add a comment... (Enter to send, Shift+Enter for new line)"
            className="flex-1 bg-white/60 border border-slate-700/60 rounded-xl px-3 py-2.5 text-xs text-slate-900 placeholder-slate-500 focus:outline-none focus:border-[#2F81F7]/60 resize-none h-[64px] transition" />
           <button onClick={handleSendComment} disabled={!commentText.trim() || commentSending}
            className="self-end px-4 py-2.5 bg-[#2F81F7] hover:bg-[#2F81F7] text-slate-900 rounded-xl flex items-center gap-1.5 text-xs font-bold disabled:opacity-40 transition">
     ...
     <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-200">
      <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-slate-200">
       <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-black text-slate-900">Create New Bug</h2>
       </div>
      ...
      <div className="shrink-0 flex justify-end gap-2 px-6 py-4 border-t border-slate-200">
     ...
     <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-200">
      <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-slate-200">
       <div className="flex items-center gap-2">
        ...
        <h2 className="text-sm font-black text-slate-900">AI Bug Generator</h2>
        <span className="text-[10px] bg-blue-950/40 border border-blue-700/40 text-blue-300 px-2 py-0.5 rounded-full font-bold">Powered by AI</span>
       </div>
       <button onClick={() => { setShowAIModal(false); setAiResult(null); setAiDesc(''); }} className="p-2 rounded-lg hover:bg-slate-800 text-slate-500"><X className="w-4 h-4" /></button>
      </div>
       ...
       {!aiResult ? (
        <>
         ...
         <div>
          ...
          <textarea value={aiDesc} onChange={e => setAiDesc(e.target.value)} rows={8}
           ...
           className="w-full bg-white/60 border border-slate-700/60 rounded-lg px-3 py-2.5 text-sm text-slate-900 placeholder-slate-500 focus:outline-none focus:border-[#2F81F7]/60 resize-none" />
         </div>
         <button onClick={handleGenerateAI} disabled={aiLoading || !aiDesc.trim()}
          className="w-full flex items-center justify-center gap-2 py-3 bg-[#2F81F7] hover:from-blue-500 hover:to-slate-500 text-slate-900 rounded-xl font-bold text-sm disabled:opacity-50 transition">
        ...
        <>
         <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={() => setAiResult(null)} className="text-xs text-slate-500 hover:text-slate-700">← Try again</button>
         </div>
         {/* Preview cards */}
         <div className="space-y-3 bg-white/30 rounded-xl p-4 border border-slate-200/50">
          <div className="flex items-center gap-2 flex-wrap">
           <SeverityBadge severity={aiResult.severity} />
           <PriorityDot priority={aiResult.priority} />
           <span className="text-xs text-slate-500">{aiResult.module}</span>
          </div>
          <h3 className="text-sm font-bold text-slate-900">{aiResult.title}</h3>
          {[
           ...
           <div key={f.label}>
            <p className={labelCls}>{f.label}</p>
            <p className="text-xs text-slate-500 whitespace-pre-wrap leading-relaxed">{f.val}</p>
           </div>
          ))}
      ...
      {aiResult && (
       <div className="shrink-0 flex justify-end gap-2 px-6 py-4 border-t border-slate-200">
        <button onClick={() => setAiResult(null)} className="px-4 py-2 text-xs font-medium text-slate-500 hover:text-slate-900">Regenerate</button>
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
     <div className="bg-white border border-rose-900/50 rounded-2xl p-6 w-full max-w-sm shadow-2xl animate-in fade-in zoom-in-95 duration-200">
      <div className="flex items-center gap-3 mb-4">
       ...
       <div>
        <h2 className="text-sm font-black text-slate-900">Delete Bug?</h2>
        <p className="text-xs text-slate-500 mt-0.5">This action cannot be undone.</p>
       </div>
      </div>
      <div className="flex justify-end gap-2 mt-4">
       <button onClick={() => setShowDeleteConfirm(false)} className="px-4 py-2 text-xs font-medium text-slate-500 hover:text-slate-900">Cancel</button>
       <button onClick={handleDeleteBug} className="px-5 py-2 bg-rose-600 hover:bg-rose-500 text-slate-900 rounded-lg text-xs font-bold transition">Delete</button>
      </div>
     </div>
    </div>
   )}
