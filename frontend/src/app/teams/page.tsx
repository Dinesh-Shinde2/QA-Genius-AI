...
const ROLE_COLORS: Record<string, string> = {
  ...
  QA_LEAD: 'text-violet-400 bg-violet-950/30 border-violet-800/50',
  QA_ENGINEER: 'text-indigo-400 bg-indigo-950/30 border-indigo-800/50',
  DEVELOPER: 'text-emerald-400 bg-emerald-950/30 border-emerald-800/50',
  TECH_LEAD: 'text-cyan-400 bg-cyan-950/30 border-cyan-800/50',
...
function TeamTypeTag({ type }: { type: string }) {
  const map: Record<string, string> = {
    QA: 'text-violet-400 bg-violet-950/25 border-violet-700/30',
    DEVELOPER: 'text-emerald-400 bg-emerald-950/25 border-emerald-700/30',
    MIXED: 'text-cyan-400 bg-cyan-950/25 border-cyan-700/30',
  };
  return (
    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${map[type] || 'text-slate-400 bg-slate-900/40 border-slate-700/30'}`}>
      {type}
    </span>
  );
}
    ...
    <div className="flex h-screen bg-[#07050d] overflow-hidden">
        ...
        <div className="w-80 border-r border-slate-900 flex flex-col shrink-0">
          <div className="p-4 border-b border-slate-900">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h1 className="text-sm font-bold text-white">Teams</h1>
                <p className="text-[10px] text-slate-500 mt-0.5">{teams.length} team{teams.length !== 1 ? 's' : ''} configured</p>
              </div>
              <button
                onClick={() => setShowCreateModal(true)}
                className="flex items-center gap-1.5 px-3 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-lg text-xs font-bold shadow-lg shadow-violet-500/20 transition active:scale-95"
              >
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {teams.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3">
                <div className="w-14 h-14 rounded-2xl bg-violet-950/30 border border-violet-800/30 flex items-center justify-center">
                ...
                <button
                  ...
                  className={`w-full text-left p-3.5 rounded-xl border transition-all duration-150 ${
                    selectedTeam?.id === team.id
                      ? 'bg-violet-950/20 border-violet-700/40 shadow-inner'
                      : 'bg-[#0e0b16]/60 border-slate-800/50 hover:border-slate-700/60 hover:bg-slate-900/20'
                  }`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                        team.team_type === 'QA' ? 'bg-violet-950/40 border border-violet-700/30' :
                        team.team_type === 'DEVELOPER' ? 'bg-emerald-950/40 border border-emerald-700/30' :
                        'bg-cyan-950/40 border border-cyan-700/30'
                      }`}>
                        <Users className={`w-4 h-4 ${
                          team.team_type === 'QA' ? 'text-violet-400' :
                          team.team_type === 'DEVELOPER' ? 'text-emerald-400' : 'text-cyan-400'
                        }`} />
                      </div>
          ...
          {!selectedTeam ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                <div className="w-20 h-20 rounded-3xl bg-violet-950/20 border border-violet-800/20 flex items-center justify-center mx-auto mb-4">
                  <Users className="w-9 h-9 text-violet-500/50" />
                </div>
                <h3 className="text-sm font-bold text-slate-400">Select a team</h3>
            ...
            <>
              {/* Team Header */}
              <div className="p-6 border-b border-slate-900 bg-[#0a0812]/50">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                      teamDetail?.team_type === 'QA' ? 'bg-violet-950/50 border border-violet-700/40' :
                      ...
                      <Users className={`w-6 h-6 ${
                        teamDetail?.team_type === 'QA' ? 'text-violet-400' :
                        teamDetail?.team_type === 'DEVELOPER' ? 'text-emerald-400' : 'text-cyan-400'
                      }`} />
                    </div>
                    <div>
                      <h2 className="text-base font-black text-white">{teamDetail?.name || selectedTeam.name}</h2>
                      ...
                      {teamDetail?.description && (
                        <p className="text-xs text-slate-400 mt-1.5">{teamDetail.description}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 bg-slate-800/60 hover:bg-slate-700/60 border border-slate-700/50 text-slate-300 rounded-lg text-xs font-semibold transition"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-lg text-xs font-bold shadow-lg shadow-violet-500/20 transition"
                    >
              ...
              <div className="flex-1 overflow-y-auto p-6">
                <div className="mb-4 flex items-center justify-between">
                  <h3 className="text-xs font-bold text-slate-300 uppercase tracking-widest">Team Members</h3>
                ...
                {!teamDetail?.members?.length ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                    {teamDetail.members.map((member: any) => {
                      ...
                      const roleColor = ROLE_COLORS[member.role_in_team || member.role] || 'text-slate-400 bg-slate-900/40 border-slate-700/30';
                      return (
                        <div key={member.id} className="bg-[#0e0b16]/80 border border-slate-800/60 rounded-xl p-4 flex items-start gap-3 hover:border-slate-700/60 transition group">
                ...
                <div className="mt-8">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-xs font-bold text-slate-300 uppercase tracking-widest">Linked Projects</h3>
                    <button
                      onClick={() => setShowAssignProjectModal(true)}
                      className="flex items-center gap-1.5 px-2 py-1 bg-violet-600/20 hover:bg-violet-600/40 text-violet-300 rounded-md text-[10px] font-bold border border-violet-500/30 transition"
                    >
                  ...
                  {teamDetail?.projects?.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {teamDetail.projects.map((p: any) => (
                        <div key={p.id} className="flex items-center gap-2 px-3 py-2 bg-slate-900/40 border border-slate-800/60 rounded-lg text-xs text-slate-300">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0e0b16] border border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-300 mb-1.5 block">Team Name *</label>
              ...
              <div>
                ...
                <textarea placeholder="Team purpose and responsibilities..." value={newTeamDesc} onChange={(e) => setNewTeamDesc(e.target.value)}
                  className="w-full bg-slate-900/60 border border-slate-700/60 rounded-lg px-3 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-violet-500/60 h-20 resize-none" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-300 mb-1.5 block">Team Type</label>
                <div className="flex gap-2">
                  {TEAM_TYPE_OPTIONS.map((t) => (
                    <button key={t} onClick={() => setNewTeamType(t)}
                      className={`flex-1 py-2 rounded-lg text-xs font-bold border transition ${newTeamType === t ? 'bg-violet-600 border-violet-500 text-white' : 'bg-slate-900/40 border-slate-700/50 text-slate-400 hover:border-slate-600'}`}>
                      {t}
                    </button>
                  ))}
                </div>
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0e0b16] border border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-black text-white">Edit Team</h2>
              <button onClick={() => setShowEditModal(false)} className="p-2 rounded-lg hover:bg-slate-800 text-slate-400"><X className="w-4 h-4" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-300 mb-1.5 block">Team Name</label>
                <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)}
                  className="w-full bg-slate-900/60 border border-slate-700/60 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-violet-500/60" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-300 mb-1.5 block">Description</label>
                <textarea value={editDesc} onChange={(e) => setEditDesc(e.target.value)}
                  className="w-full bg-slate-900/60 border border-slate-700/60 rounded-lg px-3 py-2.5 text-sm text-white focus:outline-none focus:border-violet-500/60 h-20 resize-none" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-300 mb-1.5 block">Team Type</label>
                <div className="flex gap-2">
                  {TEAM_TYPE_OPTIONS.map((t) => (
                    <button key={t} onClick={() => setEditType(t)}
                      className={`flex-1 py-2 rounded-lg text-xs font-bold border transition ${editType === t ? 'bg-violet-600 border-violet-500 text-white' : 'bg-slate-900/40 border-slate-700/50 text-slate-400 hover:border-slate-600'}`}>
                      {t}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex justify-end gap-2 mt-6">
              <button onClick={() => setShowEditModal(false)} className="px-4 py-2 text-xs font-medium text-slate-400 hover:text-white">Cancel</button>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0e0b16] border border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => { setShowAddMemberModal(false); setUserQuery(''); setUserResults([]); }} className="p-2 rounded-lg hover:bg-slate-800 text-slate-400"><X className="w-4 h-4" /></button>
            </div>
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-900/60 border border-slate-700/60 rounded-lg pl-9 pr-3 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-violet-500/60"
              />
            </div>
            <div className="mt-3 space-y-1 max-h-64 overflow-y-auto">
              ...
              {userResults.map((u) => {
                ...
                return (
                  <div key={u.id} className={`flex items-center gap-3 p-3 rounded-xl border transition ${isAlreadyMember ? 'border-slate-800/30 opacity-50' : 'border-slate-800/60 hover:border-violet-700/40 hover:bg-violet-950/10 cursor-pointer'}`}
                    ...
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${ROLE_COLORS[u.role] || 'text-slate-400 bg-slate-900/40 border-slate-700/30'}`}>
                      {u.role?.replace(/_/g, ' ')}
                    </span>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0e0b16] border border-rose-900/50 rounded-2xl p-6 w-full max-w-sm shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-sm font-black text-white">Delete Team?</h2>
                <p className="text-xs text-slate-400 mt-0.5">This action cannot be undone.</p>
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setShowDeleteConfirm(null)} className="px-4 py-2 text-xs font-medium text-slate-400 hover:text-white">Cancel</button>
              <button onClick={() => handleDeleteTeam(showDeleteConfirm)}
                className="px-5 py-2 bg-rose-600 hover:bg-rose-500 text-white rounded-lg text-xs font-bold transition">
                Delete Team
              </button>
            </div>
          </div>
      ...
      {showAssignProjectModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#0e0b16] border border-slate-800 rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-base font-black text-white">Assign Team to Project</h2>
              <button onClick={() => setShowAssignProjectModal(false)} className="p-2 rounded-lg hover:bg-slate-800 text-slate-400"><X className="w-4 h-4" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-300 mb-1.5 block">Select Project</label>
                <select
            ...
            <div className="flex justify-end gap-2 mt-6">
              <button onClick={() => setShowAssignProjectModal(false)} className="px-4 py-2 text-xs font-medium text-slate-400 hover:text-white">Cancel</button>
